...
public class ExtractTopPersonalizedPageRankNodes extends Configured implements Tool {
  ...
  private static class MyMapper extends
    ...
    public void setup(Context context) throws IOException {
      ...
      sourcecount = context.getConfiguration().getInt("NumSources", 1);
      queue = new TopScoredObjects[sourcecount];
  ...
  public int run(String[] args) throws Exception {
    ...
    conf.setInt("n", n);
    conf.setStrings("SourceNodes", sources);
    ...
    while((line = br.readLine()) != null) {
      if (count % n == 0) {
        System.out.println();
        System.out.println("Source:\t" + sourceList[count / n]);
      }
      String[] tokens = line.split("\\t");
      pagerank = Float.parseFloat(tokens[0]);
