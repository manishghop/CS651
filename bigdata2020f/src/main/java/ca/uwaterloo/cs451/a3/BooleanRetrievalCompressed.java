...
public class BooleanRetrievalCompressed extends Configured implements Tool {
  ...
  private ArrayListWritable<PairOfInts> fetchPostings(String term) throws IOException {
    ...
    FileStatus[] status=f.listStatus(new Path(p));
    for(FileStatus fs:status)
    {
    String fil=fs.getPath().toString();
    if(fil.contains("_SUCCESS")) continue;
    Path path=new Path(fil);
    index=new MapFile.Reader(path,f.getConf());
   
    key.set(term);
    index.get(key, value);
    if(value.getLength()==0) continue;
    
    ByteArrayInputStream bs=new ByteArrayInputStream(value.getBytes());
    DataInputStream ds=new DataInputStream(bs);
    int df= WritableUtils.readVInt(ds);
    int tf;
    int docid=0;
    for(int i=0;i<df;i++)
    
    {
    docid+=WritableUtils.readVInt(ds);
